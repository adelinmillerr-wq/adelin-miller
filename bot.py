...
@router.message(Command("emergency_cleanup"))
async def cmd_emergency_cleanup(message: Message, bot: Bot):
    """Аварийная очистка массовой ложной выдачи.

    Ищет только записи, которые были созданы ДО cutoff, но получили доступ
    ПОСЛЕ cutoff, при этом operation_id совпадает с label. Это характерный
    след старой ошибочной повторной обработки. Отзывает invite и удаляет
    уже вошедшего по этой ложной ссылке пользователя.
    """
    if not _is_admin(message):
        return
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) < 2:
        await message.answer(
            "Пример: <code>/emergency_cleanup 2026-09-07T14:48</code>\n\n"
            "Команда ищет именно ложные выдачи, возникшие после указанного времени."
        )
        return

    try:
        cutoff = datetime.fromisoformat(parts[1].strip())
    except ValueError:
        await message.answer("Неверная дата. Формат: <code>2026-09-07T14:48</code>")
        return
    ...
    try:
        conn = get_db()
        rows = conn.run(
            """SELECT label, user_id, username, invite_link, created_at,
                      paid_at, access_start_dt, operation_id
               FROM yoomoney_attempts
               WHERE access_granted=TRUE
                 AND invite_link IS NOT NULL
                 AND created_at < :cutoff
                 AND access_start_dt >= :cutoff
                 AND operation_id = label
               ORDER BY access_start_dt""",
            cutoff=cutoff,
        )
    ...
    if not rows:
        await message.answer(
            "🔎 Ложных выдач по этому критерию не найдено.\n\n"
            "Ничего не удаляла и не отзывала."
        )
        return
    ...
    for label, user_id, username, invite_link, created_at, paid_at, access_start_dt, operation_id in rows:
        # Сначала закрываем саму ссылку.
        try:
            await bot.revoke_chat_invite_link(CLUB_CHAT_ID, invite_link)
            revoked += 1
        except Exception as e:
            revoke_failed += 1
            log.warning("cleanup: не смог отозвать %s: %s", label, e)

        # Затем удаляем пользователя, если он уже вошёл.
        try:
            await bot.ban_chat_member(chat_id=CLUB_CHAT_ID, user_id=int(user_id))
            await bot.unban_chat_member(chat_id=CLUB_CHAT_ID, user_id=int(user_id))
            removed += 1
            ...
            remove_failed += 1
            log.warning("cleanup: не смог удалить %s/%s: %s", label, user_id, e)

        affected.append(f"@{username or '—'} ({user_id})")

    # Ложные выдачи больше не должны выглядеть как активный доступ в БД.
    conn = None
    try:
        conn = get_db()
        for label, *_rest in rows:
            conn.run(
                """UPDATE yoomoney_attempts
                   SET access_granted=FALSE, user_notified=TRUE, admin_notified=TRUE
                   WHERE label=:label""",
                label=label,
            )
            conn.run(
                """UPDATE members
                   SET status='revoked_fake_payment', in_chat=FALSE, chat_status='removed_fake_payment'
                   WHERE user_id=(SELECT user_id FROM yoomoney_attempts WHERE label=:label)""",
                label=label,
            )
    except Exception as e:
        log.error("cleanup: не смог пометить записи: %s", e)
    finally:
        if conn:
            conn.close()
    ...
    preview = "\n".join(affected[:30])
    if len(affected) > 30:
        preview += f"\n… и ещё {len(affected)-30}"

    await message.answer(
        "🚨 <b>АВАРИЙНАЯ ОЧИСТКА ЗАВЕРШЕНА</b>\n\n"
        f"Найдено ложных выдач: <b>{len(rows)}</b>\n"
        f"🔗 Ссылок отозвано: <b>{revoked}</b> (ошибок: {revoke_failed})\n"
        f"🚪 Уже вошедших удалено: <b>{removed}</b> (ошибок: {remove_failed})\n\n"
        "<b>Затронутые пользователи:</b>\n" + preview
    )


@router.message(Command("revoke_legacy"))
async def cmd_revoke_legacy(message: Message, bot: Bot):
    """Отзывает invite-ссылки, созданные для старых платежей до указанного времени.

    Пример: /revoke_legacy 2026-09-07T14:50
    Это НЕ удаляет уже вошедших участников. Отзывается только ссылка.
    """
    if not _is_admin(message):
        return
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) < 2:
        await message.answer(
            "Пример: <code>/revoke_legacy 2026-09-07T14:50</code>\n\n"
            "Укажи время ДО которого нужно отозвать сохранённые invite-ссылки."
        )
        return
    raw = parts[1].strip()
    try:
        ...
        await message.answer("Неверная дата. Формат: <code>2026-09-07T14:50</code>")
        return
    ...
    try:
        conn = get_db()
        rows = conn.run(
            """SELECT label, invite_link, user_id, username, paid_at
               FROM yoomoney_attempts
               WHERE access_granted=TRUE AND invite_link IS NOT NULL
    ...
    revoked = 0
    failed = 0
    for label, invite_link, user_id, username, paid_at in rows:
        try:
            await bot.revoke_chat_invite_link(CLUB_CHAT_ID, invite_link)
            revoked += 1
        except Exception as e:
            failed += 1
            log.warning("Не смог отозвать ссылку %s: %s", label, e)

    await message.answer(
        f"🧹 <b>Готово</b>\n\n"
        f"Найдено старых ссылок: {len(rows)}\n"
        f"Отозвано: {revoked}\n"
        f"Не удалось отозвать: {failed}\n\n"
        "Уже вступивших пользователей эта команда не удаляет."
    )
...
async def handle_yoomoney_notification(request: web.Request) -> web.Response:
    ...
    # КРИТИЧЕСКОЕ ПРАВИЛО: доступ выдаём только тому webhook,
    # который первым перевёл конкретный платеж из pending -> paid.
    # Уже существующий status=paid никогда повторно не вызывает grant_access.
    if info.get("status") != "pending":
        log.info(
            "ЮMoney: повторное/старое уведомление для %s, статус=%s. Доступ повторно не выдаём.",
            label, info.get("status"),
        )
        return web.Response(status=200, text="OK")

    marked = mark_yoomoney_payment_received(label, operation_id, paid)
    if not marked:
        # Другой webhook мог забрать этот платеж первым. Ничего повторно не выдаём.
        refreshed = load_yoomoney_attempt(label)
        if refreshed and refreshed.get("status") == "paid":
            log.info("ЮMoney: %s уже обработан другим запросом.", label)
            return web.Response(status=200, text="OK")
...
async def main():
    ...
    # После запуска только сверяем известных покупателей. Доступ из sync НИКОГДА не выдаётся.
    try:
