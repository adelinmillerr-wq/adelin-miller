...
# Секретный хвост в URL для приема вебхуков от Lava.top
...
def create_lava_invoice(chat_id, tariff_id):
    """Динамическое создание официального счета через Public API Lava.top"""
    t = TARIFFS[tariff_id]
    ...
    payload = {
        "amount": int(t['price']),
        "comment": f"Creator Lab — {t['name']}",
        "fail_url": "https://t.me/share/url?url=Попробуйте еще раз",
        "custom_fields": {
            "utm_content": f"{chat_id}_{tariff_id}"
        }
    }
    try:
        r = requests.post(url, json=payload, headers=headers, timeout=10)
        res_data = r.json()
        if r.status_code in [200, 201] and 'data' in res_data:
            return res_data['data'].get('url') or res_data['data'].get('url_page')
    except Exception as e:
        logger.error(f"Error creating Lava invoice via API: {e}")
    return None
...
def lavatop_result():
    ...
    # Вытаскиваем utm_content из кастомных полей API инвойса
    custom_fields = payload_data.get('custom_fields', {}) or data.get('custom_fields', {})
    utm_content = custom_fields.get('utm_content') or payload_data.get('utm_content')
    
    if not utm_content and 'order' in payload_data:
        order_custom = payload_data['order'].get('custom_fields', {}) or {}
        utm_content = order_custom.get('utm_content') or payload_data['order'].get('utm_content')

    logger.info(f"Extracted utm_content from API: {utm_content}")
    ...
    if utm_content and '_' in str(utm_content):
        ...
        logger.warning("Webhook received but no valid data found.")
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Генерируем официальные красивые ссылки динамически
    url_lava1 = create_lava_invoice(chat_id, 1)
    url_lava2 = create_lava_invoice(chat_id, 2)
    url_lava3 = create_lava_invoice(chat_id, 3)
    ...
    if url_lava1:
        keyboard.append([InlineKeyboardButton("💜 1 месяц — 690 руб (АКЦИЯ)", url=url_lava1)])
    if url_lava2:
        keyboard.append([InlineKeyboardButton("💜 Навсегда — 8990 руб (ВЫГОДНО)", url=url_lava2)])
    if url_lava3:
        keyboard.append([InlineKeyboardButton("🧪 Тест — 50 руб", url=url_lava3)])
