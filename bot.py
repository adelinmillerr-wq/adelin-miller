...
IS_TEST      = os.environ.get('IS_TEST', '0')
...
def make_inv_id(chat_id, tariff_id):
    ...
    conn = get_db()
    conn.run('INSERT INTO invoices VALUES (:inv_id,:chat_id,:tariff_id,:created) ON CONFLICT DO NOTHING',
             inv_id=inv_id, chat_id=chat_id, tariff_id=tariff_id, created=datetime.now())
    conn.close()
    return inv_id
...
def get_usdt_rate():
    ...
    try:
        ...
        return 72.0  # fallback курс


def rub_to_usdt(rub_amount):
    """Конвертируем рубли в USDT с учётом комиссии 3%"""
    rate = get_usdt_rate()
    usdt = (rub_amount / rate) * 1.03
    return math.ceil(usdt * 100) / 100  # округляем вверх до 2 знаков
...
def robokassa_result():
    ...
    logger.error(f"SIGN CHECK: got={sig.upper()}, expected={expected}, sum={out_sum}, inv={inv_id}, pass2_len={len(RK_PASSWORD2) if RK_PASSWORD2 else 0}, shp={shp}")
...
def cryptobot_result():
    """Вебхук от CryptoBot после оплаты"""
    data = request.json
...
async def process_payment(chat_id, tariff_id, payment_type='rub'):
    t = TARIFFS[tariff_id]
    ...
    pay_method = "💎 Крипта (USDT)" if payment_type == 'crypto' else "💳 Рублями"
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Получаем курс для крипты
    try:
        usdt1 = rub_to_usdt(690)
        usdt2 = rub_to_usdt(8990)
        crypto_btn1 = f"💎 1 месяц — {usdt1} USDT (крипта)"
        crypto_btn2 = f"💎 Навсегда — {usdt2} USDT (крипта)"
        url_crypto1, _ = create_crypto_invoice(chat_id, 1)
        url_crypto2, _ = create_crypto_invoice(chat_id, 2)
    except Exception as e:
        logger.error(f"Crypto error: {e}")
        url_crypto1 = url_crypto2 = None
        crypto_btn1 = "💎 1 месяц — крипта (USDT)"
        crypto_btn2 = "💎 Навсегда — крипта (USDT)"
