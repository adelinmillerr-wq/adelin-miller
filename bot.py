...
# Значения цен по умолчанию (если в базе еще не переопределены)
DEFAULT_PRICES = {
...
ASK_EMAIL = (
    ...
    "На неё придёт информация и чек. Просто отправь её сообщением."
)
...
def get_price(tariff_key: str) -> int:
    """Получает актуальную цену из базы данных или берет дефолтную."""
    if not DATABASE_URL:
...
async def cmd_help(message: Message):
    ...
    await message.answer(
        ...
        "/members — полный список участников\n"
        ...
        "/extend @username 30 — продлить доступ\n"
        "/remove @username — кикнуть из клуба\n"
        "/ban @username — добавить в ЧС\n"
        "/unban @username — снять из ЧС"
    )


# ---------------------------------------------------------------------------
# Сценарий покупки и диалог в Telegram
# ---------------------------------------------------------------------------
...
async def on_email(message: Message, state: FSMContext):
    ...
    price = get_price(tariff_key)
    label = f"ym_{user.id}_{uuid.uuid4().hex[:12]}"
    
    # Запоминаем намерение оплаты
...
# ---------------------------------------------------------------------------
# ЮMoney: форма оплаты + Обработка HTTP-уведомлений (Вебхуков)
# ---------------------------------------------------------------------------
...
def verify_yoomoney_sign(params: dict[str, str]) -> bool:
    ...
    # Формирование строки подписи согласно официальной спецификации ЮMoney:
...
async def main():
    ...
    # Роуты для оплаты и уведомлений ЮMoney
