...
DATABASE_URL = os.environ.get('DATABASE_URL', '').replace('postgresql://', 'postgresql://')
OWNER_ID     = 1619432734
...
def make_inv_id_sync(chat_id, tariff_id):
    ts = int(datetime.now().timestamp())
    return int(str(tariff_id) + str(abs(chat_id) % 100000) + str(ts % 10000))


async def make_inv_id(chat_id, tariff_id):
    inv_id = make_inv_id_sync(chat_id, tariff_id)
    async with db_pool.acquire() as conn:
        await conn.execute(
            'INSERT INTO invoices VALUES ($1,$2,$3,$4) ON CONFLICT DO NOTHING',
            inv_id, chat_id, tariff_id, datetime.now()
        )
    return inv_id


def robokassa_url_sync(chat_id, tariff_id):
    t = TARIFFS[tariff_id]
    inv_id = make_inv_id_sync(chat_id, tariff_id)
...
async def process_payment(chat_id, tariff_id):
    ...
    inv_id = await make_inv_id(chat_id, tariff_id)

    async with db_pool.acquire() as conn:
        await conn.execute('''INSERT INTO subscriptions (chat_id, tariff_id, start_dt, end_dt, status)
                              VALUES ($1,$2,$3,$4,'active')
                              ON CONFLICT (chat_id) DO UPDATE SET
                              tariff_id=$2, start_dt=$3, end_dt=$4, status='active' ''',
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute('''INSERT INTO subscriptions (chat_id, username, tariff_id, start_dt, status)
                                  VALUES ($1,$2,0,NOW(),'')
    ...
    url1 = robokassa_url_sync(chat_id, 1)
    url2 = robokassa_url_sync(chat_id, 2)
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
            'SELECT tariff_id, end_dt, status FROM subscriptions WHERE chat_id=$1', chat_id)
    ...
    for row in rows:
        ...
        if delta.total_seconds() <= 0:
            try:
                ...
                async with db_pool.acquire() as conn:
                    await conn.execute(
                        "UPDATE subscriptions SET status='expired' WHERE chat_id=$1", chat_id)
            except Exception as e:
