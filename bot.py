...
def init_db():
    ...
    try:
        ...
        conn.close()
        log.info("База готова, таблицы members, consents, pending_members, blacklisted_users на месте.")
    except Exception as e:  # noqa: BLE001
...
async def cmd_ban(message: Message, bot: Bot):
    ...
    if uid is not None:
        try:
            await bot.send_message(uid, BLACKLIST_NOTICE)
            notice_line = "\nПользователю отправлено уведомление."
        except Exception as e:  # noqa: BLE001
            log.warning("Не смог отправить уведомление о бане %s: %s", uid, e)
            notice_line = "\nВ черный список добавлен, но уведомление отправить не вышло."
    else:
        notice_line = "\nПока знаю только username, поэтому уведомление отправить не могу."
