...
GREETING = (
    ...
    "Оплата — один раз, доступ — навсегда. Никаких продлений и подписок 👇\n\n"
...
def init_db():
    ...
    try:
        ...
        # Миграция ранее созданных ботом ссылок. Благодаря этому кнопка
        # аварийного сброса сможет отозвать ссылки, которые уже лежали
        # в yoomoney_attempts до появления таблицы invite_links.
        conn.run(
            """INSERT INTO invite_links (label, user_id, invite_link, created_at, revoked, source)
               ...
               WHERE ya.invite_link IS NOT NULL
                 AND ya.invite_link <> ''
        ...
        conn.run("CREATE INDEX IF NOT EXISTS idx_invite_links_revoked ON invite_links(revoked)")
...
# ---------------------------------------------------------------------------
# Invite links: lifecycle and emergency cleanup
# ---------------------------------------------------------------------------


def save_invite_link(label: str, user_id: int, invite_link: str, source: str = "payment"):
    """Сохраняет каждую ссылку, которую создал бот, чтобы её можно было отозвать позже."""
    ...
    try:
        ...
        conn.run(
            """INSERT INTO invite_links
               (label, user_id, invite_link, created_at, revoked, source)
...
async def revoke_invite_link(bot: Bot, invite_link: str, joined_user_id: Optional[int] = None) -> bool:
    """Отзывает только ссылку, которую создал этот бот."""
    if not invite_link:
        return False
    try:
        ...
        # Ссылка уже могла стать неактивной из-за member_limit=1.
        err = str(e).lower()
...
async def revoke_all_bot_invites(bot: Bot) -> dict:
    """Отзывает все НЕотозванные ссылки, которые записаны в нашей БД.

    Telegram Bot API не предоставляет боту метод 'получить все созданные мной ссылки'.
    Поэтому надёжная массовая очистка возможна только для ссылок, которые бот сохранил.
    """
    ...
    try:
        conn = get_db()
        rows = conn.run(
            "SELECT invite_link FROM invite_links WHERE revoked=FALSE ORDER BY id"
        )
    except Exception as e:  # noqa: BLE001
...
async def on_chat_member_update(event: ChatMemberUpdated):
    ...
    # Если человек вошёл по ссылке, которую создал наш бот, немедленно отзываем её.
    # Это дополнительная защита поверх member_limit=1.
    if in_chat and event.invite_link:
        ...
        bot_is_creator = bool(creator and getattr(creator, "id", None) == bot.id)
        if invite_url and bot_is_creator:
            revoked = await revoke_invite_link(bot, invite_url, joined_user_id=user_id)
            log.info("Invite link после входа %s: revoked=%s, user=%s", invite_url, revoked, user_id)
...
# Повторная автоматическая выдача старых paid-записей намеренно отключена.
# Доступ выдаётся только непосредственно из подтверждённого нового webhook ЮMoney.
...
def admin_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(
                text="🧹 Сбросить созданные ботом ссылки",
                callback_data="admin_revoke_invites",
            )],
        ]
    )
...
async def on_admin_revoke_invites(cb: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        await cb.message.answer(
            ...
            f"Найдено активных ссылок в базе: <b>{result['found']}</b>\n"
            f"Отозвано: <b>{result['revoked']}</b>\n"
            f"Не удалось отозвать: <b>{result['failed']}</b>\n\n"
            "⚠️ Людей эта кнопка не удаляет. Она только закрывает ссылки для новых входов.",
            reply_markup=admin_keyboard(),
        )
    except Exception as e:  # noqa: BLE001
        log.exception("Ошибка массового сброса invite links: %s", e)
        await cb.message.answer(
            f"❌ Ошибка сброса ссылок: {e}",
            reply_markup=admin_keyboard(),
        )
...
async def cmd_revoke_invites(message: Message, bot: Bot):
    if not _is_admin(message):
        return
    await message.answer("🧹 Отзываю все сохранённые invite-ссылки, созданные ботом…")
    result = await revoke_all_bot_invites(bot)
    await message.answer(
        "🧹 <b>Сброс ссылок завершён</b>\n\n"
        f"Найдено активных ссылок в базе: <b>{result['found']}</b>\n"
...
async def cmd_resend(message: Message, bot: Bot):
    ...
    if _old_link:
        await revoke_invite_link(bot, _old_link)
    try:
        ...
        save_invite_link(label, uid, invite_link, source="resend")
...
async def cmd_help(message: Message):
    ...
    await message.answer(
        ...
        "/resend @username — повторно отправить ссылку\n"
        "/revoke_invites — отозвать сохранённые ссылки бота\n"
    )
...
async def grant_access(
    ...
    if not invite_url:
        try:
            ...
            save_invite_link(label, user_id, invite_url, source="payment")
        except Exception as e:  # noqa: BLE001
