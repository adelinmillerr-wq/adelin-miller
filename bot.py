...
def yoomoney_result():
    ...
    # withdraw_amount — сколько списано с покупателя (до комиссии). Если его нет, берём amount.
    withdraw = float(data.get('withdraw_amount', 0)) or amount

    logger.info(f"YooMoney: label={label}, amount={amount}, withdraw={withdraw}")
    ...
    if label and '_' in label:
        try:
            ...
            asyncio.run_coroutine_threadsafe(
                process_payment(chat_id, tariff_id, payment_type='yoomoney', paid_amount=withdraw),
                main_loop
            )
        except Exception as e:
...
async def process_payment(chat_id, tariff_id, payment_type='yoomoney', paid_amount=None):
    t = TARIFFS[tariff_id]
    b = Bot(token=BOT_TOKEN)

    # Защита от оплаты по старой ссылке: если через ЮMoney пришло заметно меньше актуальной цены — не пускаем.
    # Допуск 5% — чтобы комиссия ЮMoney (~3%) не считалась недоплатой.
    if payment_type == 'yoomoney' and paid_amount is not None and paid_amount < t['price'] * 0.95:
        try:
            await b.send_message(
                chat_id,
                f"⚠️ Цена доступа в клуб изменилась.\n\n"
                f"Ты оплатил(а) по старой ссылке ({int(paid_amount)}₽), а она больше не действует. "
                f"Актуальная цена тарифа «{t['name']}» — {t['price']}₽.\n\n"
                f"Доступ по старой цене открыть не получится. "
                f"Чтобы вступить, оформи оплату заново по актуальной цене через /start.\n\n"
                f"По возврату старого платежа напиши @adelin_pro 💛"
            )
        except Exception as e:
            logger.error(f"Underpay notify error: {e}")
        # уведомление тебе
        try:
            await b.send_message(
                OWNER_ID,
                f"❗️ Оплата по СТАРОЙ ссылке\n\n"
                f"Тариф: {t['name']} (актуально {t['price']}₽)\n"
                f"Пришло: {int(paid_amount)}₽\n"
                f"chat_id: {chat_id}\n\n"
                f"Доступ НЕ выдан. Человеку сообщено, что цена изменилась.\n"
                f"Не забудь вернуть ему старый платёж или предложить доплату."
            )
        except Exception as e:
            logger.error(f"Owner notify error: {e}")
        return  # доступ не выдаём
