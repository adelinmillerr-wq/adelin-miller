...
# Auth for lava.top webhooks. Prefer env vars in production.
LAVA_WEBHOOK_LOGIN = os.environ.get('LAVA_WEBHOOK_LOGIN', 'adelin')
LAVA_WEBHOOK_PASSWORD = os.environ.get('LAVA_WEBHOOK_PASSWORD', 'Albina1991!')
LAVA_WEBHOOK_API_KEY = (
    os.environ.get('LAVA_WEBHOOK_API_KEY')
    or os.environ.get('LAVA_WEBHOOK_SECRET')
    or os.environ.get('LAVA_API_KEY')
)

TARIFFS = {
    1: {'name': '1 месяц',  'price': 690,  'days': 30},
    2: {'name': 'Навсегда', 'price': 8990, 'days': None},
}
...
def make_lava_url(chat_id, tariff_id):
    ...
    return f"{base_url}{separator}utm_content={chat_id}_{tariff_id}"


LAVA_SUCCESS_EVENTS = {'payment.success', 'subscription.recurring.payment.success'}
LAVA_SUCCESS_STATUSES = {'success', 'paid', 'completed', 'subscription-active'}


def safe_compare(expected, actual):
    return bool(expected) and bool(actual) and hmac.compare_digest(str(expected), str(actual))


def is_lava_authorized():
    x_api_key = request.headers.get('X-Api-Key', '')
    if safe_compare(LAVA_WEBHOOK_API_KEY, x_api_key):
        return True

    auth = request.authorization
    if auth and safe_compare(LAVA_WEBHOOK_LOGIN, auth.username) and safe_compare(LAVA_WEBHOOK_PASSWORD, auth.password):
        return True

    return False


def extract_lava_utm_content(data):
    for container_key in ('clientUtm', 'clientUTM', 'client_utm'):
        client_utm = data.get(container_key) or {}
        if isinstance(client_utm, dict):
            for key in ('utm_content', 'utmContent'):
                value = client_utm.get(key)
                if value:
                    return str(value)
...
def get_lava_buyer_email(data):
    buyer = data.get('buyer') or {}
    if isinstance(buyer, dict) and buyer.get('email'):
        return buyer.get('email')
    return data.get('buyer_email') or data.get('buyerEmail') or ''


async def notify_owner_lava_problem(data, reason):
    b = Bot(token=BOT_TOKEN)
    buyer_email = get_lava_buyer_email(data)
    ...
    client_utm = data.get('clientUtm') or data.get('clientUTM') or data.get('client_utm') or ''

    text = (
        "Lava.top прислала успешную оплату, но бот не смог выдать доступ.\n\n"
        f"Причина: {reason}\n"
        f"eventType: {data.get('eventType', '')}\n"
        f"status: {data.get('status', '')}\n"
        f"Покупатель: {buyer_email}\n"
        f"Продукт: {product_title} {product_id}\n"
        f"contractId: {data.get('contractId', '')}\n"
        f"clientUtm: {client_utm}"
    )
    await b.send_message(chat_id=OWNER_ID, text=text[:3900])


def schedule_owner_lava_problem(data, reason):
    if not main_loop:
        logger.error("Lava.top: cannot notify owner, bot loop is not ready")
        return
    future = asyncio.run_coroutine_threadsafe(notify_owner_lava_problem(data, reason), main_loop)
...
def log_future_exception(future):
    try:
        ...
        return
    if exc:
        logger.error(f"Async task failed: {exc}")


@app.route('/lava/result', methods=['POST'])
def lavatop_result():
    """Вебхук от lava.top после оплаты."""
    if not is_lava_authorized():
        logger.warning(
            "Lava.top: неверная авторизация basic=%s x_api_key=%s",
            bool(request.authorization), bool(request.headers.get('X-Api-Key'))
        )
    ...
    if event_type:
        if event_type not in LAVA_SUCCESS_EVENTS:
            return 'ok'
    elif status not in LAVA_SUCCESS_STATUSES:
        return 'ok'

    utm_content = extract_lava_utm_content(data)
    if not utm_content:
        reason = "не найден clientUtm.utm_content с chat_id и tariff_id"
        logger.error(f"Lava.top: {reason}. Payload: {data}")
        schedule_owner_lava_problem(data, reason)
        return 'ok'

    try:
        chat_id_str, tariff_id_str = utm_content.split('_', 1)
        chat_id = int(chat_id_str)
        tariff_id = int(tariff_id_str)
        if tariff_id not in TARIFFS:
            raise ValueError(f"unknown tariff_id={tariff_id}")
        if not main_loop:
            logger.error("Lava.top: bot loop is not ready")
        ...
        reason = f"не удалось разобрать utm_content={utm_content}: {e}"
        logger.error(f"Lava parse error: {reason}")
        schedule_owner_lava_problem(data, reason)
...
async def process_payment(chat_id, tariff_id, payment_type='lava'):
    t = TARIFFS[tariff_id]
    b = Bot(token=BOT_TOKEN)
    ...
    conn = get_db()
    conn.run('''INSERT INTO subscriptions (chat_id, username, tariff_id, start_dt, end_dt, status)
                VALUES (:chat_id,:username,:tariff_id,:start_dt,:end_dt,'active')
                ON CONFLICT (chat_id) DO UPDATE SET
                username=:username, tariff_id=:tariff_id, start_dt=:start_dt, end_dt=:end_dt, status='active' ''',
             chat_id=chat_id, username=username, tariff_id=tariff_id, start_dt=start_dt, end_dt=end_dt)
    conn.close()

    pay_method = "💎 Крипта (USDT)" if payment_type == 'crypto' else "💳 Lava.top"
