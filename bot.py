...
def init_db():
    ...
    try:
        ...
        conn.close()
        log.info("База готова, таблицы members и consents на месте.")
    except Exception as e:  # noqa: BLE001
...
async def daily_report(bot: Bot):
    """Раз в сутки шлём админу сводку — доказательство, что бот следит."""
    try:
        text = await build_status_text()
        await bot.send_message(ADMIN_ID, "☀️ <b>Ежедневный отчёт</b>\n\n" + text)
    except Exception as e:  # noqa: BLE001
...
async def main():
    ...
    scheduler.add_job(daily_report, "interval", hours=24, args=[bot])
    scheduler.start()
