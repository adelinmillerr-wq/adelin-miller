...
async def build_status_text() -> str:
    ...
    try:
        conn = get_db()
        active = conn.run(
            "SELECT username, end_dt, user_id FROM members "
    ...
    if active:
        ...
        for username, end_dt, user_id in active[:10]:  # первые 10 по дате
            days_left = (end_dt - now).total_seconds() / 86400
            mark = "⚠️" if days_left <= 3 else "🔹"
            if username:
                who = f"@{username} (id <code>{user_id}</code>)"
            else:
                # ника нет — даём кликабельную ссылку по id, чтобы человек не терялся
                who = f'<a href="tg://user?id={user_id}">без ника</a> (id <code>{user_id}</code>)'
            lines.append(
                f"{mark} {who} — до {end_dt.strftime('%d.%m.%Y')} "
...
async def cmd_members(message: Message):
    ...
    lines = [f"👥 <b>Участники клуба — всего {len(rows)}</b>\n"]
    ...
    if expired:
        lines.append(f"\n❌ <b>Истёкшие ({len(expired)}):</b>")
        for i, (username, end_dt) in enumerate(expired, 1):
            d = end_dt.strftime('%d.%m.%Y') if end_dt else "—"
            lines.append(f"{i}. ❌ @{username or '—'} — истёк {d}")
