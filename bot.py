...
async def cmd_help(message: Message):
    ...
    await message.answer(
        ...
        "/sync — сверить базу с группой и убрать призраков\n\n"
...
async def cmd_sync(message: Message, bot: Bot):
    ...
    for user_id, username, tariff in rows:
        ...
        try:
            ...
            # статусы 'left' (вышел) и 'kicked' (забанен) = человека нет в группе
            if member.status in ("left", "kicked"):
                ghosts.append((user_id, username, tariff))
        except Exception:  # noqa: BLE001
            # если бот не смог проверить (например, user_id неизвестен группе) — тоже призрак
            ghosts.append((user_id, username, tariff))
            errors += 1
    ...
    if not ghosts:
        await message.answer(
            f"✅ Готово! Проверено {checked}, все на месте. Призраков нет 🎉")
        return
