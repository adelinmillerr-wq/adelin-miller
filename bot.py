...
TARIFFS = {
    ...
    3: {'name': 'Пробный период 1 неделя', 'price': 450, 'days': 7},
}
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if url_crypto_week:
        keyboard.append([InlineKeyboardButton(f"💎 Пробная неделя — {usdt_week} USDT (крипта)", url=url_crypto_week)])
    if url_crypto1:
    ...
    # ЮMoney кнопки
    keyboard.append([InlineKeyboardButton(f"💛 Пробный период 1 неделя — {TARIFFS[3]['price']} руб (ЮMoney)", url=make_ym_url(chat_id, 3))])
