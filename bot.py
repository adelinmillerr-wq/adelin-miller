...
# Возвращаем проверенные красивые страницы продуктов, которые работают без ошибок
LAVA_URLS = {
...
def make_lava_url(chat_id, tariff_id):
    """Чистая ссылка без ломающих параметров"""
    return LAVA_URLS.get(tariff_id, '')
...
def lavatop_result():
    ...
    # Вытягиваем email или внутренний ID заказа, если utm_content пустой
    ...
    # Если платеж прошел без меток (прямой по ссылке), мы подстрахуемся и выведем лог
    if utm_content and '_' in str(utm_content):
        ...
        logger.warning("Webhook received but no valid utm_content found. Проверь email покупателя в личном кабинете Lava.")
